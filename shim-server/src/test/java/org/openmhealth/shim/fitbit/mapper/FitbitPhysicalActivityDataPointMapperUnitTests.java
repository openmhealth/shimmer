...
public class FitbitPhysicalActivityDataPointMapperUnitTests extends DataPointMapperUnitTests {
    ...
    JsonNode responseNodeSingleActivity,responseNodeUserInfo,responseNodeMultipleActivities,responseNodeEmptyActivities;
    ...
    public void asDataPointsShouldReturnCorrectNumberOfDataPoints(){
        List<DataPoint<PhysicalActivity>> dataPoints = mapper.asDataPoints(Lists.newArrayList(responseNodeUserInfo, responseNodeSingleActivity));
        assertThat(dataPoints.size(),equalTo(1));
        dataPoints = mapper.asDataPoints(Lists.newArrayList(responseNodeUserInfo,responseNodeMultipleActivities));
        assertThat(dataPoints.size(),equalTo(3));
        dataPoints = mapper.asDataPoints(Lists.newArrayList(responseNodeUserInfo,responseNodeEmptyActivities));
    ...
    public void asDataPointsShouldReturnCorrectDataPointsForMultipleActivities(){
        List<DataPoint<PhysicalActivity>> dataPoints = mapper.asDataPoints(Lists.newArrayList(responseNodeUserInfo, responseNodeMultipleActivities));
    ...
    public void testFitbitPhysicalActivityDataPoint(DataPoint<PhysicalActivity> dataPoint,String activityName,String dateString,String startTimeString, Double distance,Long durationInMillis,Long logId){
        ...
        if(startTimeString!=null && durationInMillis!=null){
            OffsetDateTime offsetStartDateTime = OffsetDateTime.parse(dateString + "T" + startTimeString + "-04:00", DateTimeFormatter.ISO_OFFSET_DATE_TIME);
            ...
            OffsetDateTime offsetStartDateTime = OffsetDateTime.parse(dateString + "T" + "00:00:00-04:00",DateTimeFormatter.ISO_OFFSET_DATE_TIME);
